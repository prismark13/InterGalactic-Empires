...
class GameCore {
    ...
    generateLounge() {
        ...
        for(let i=0; i<3; i++) {
            ...
            let nameList = DATA.NAMES.Spacer || []; // Fallback

            // Smart Role Logic with Safety Check
            if (DATA.NAMES.Spacer) {
                if(role==="Scientist" || role==="Medic") nameList = DATA.NAMES.HighBorn;
                ...
                else nameList = [...DATA.NAMES.Spacer, ...DATA.NAMES.HighBorn]; // Pilot
            } else if (Array.isArray(DATA.NAMES)) {
                nameList = DATA.NAMES; // Handle flat array legacy case
            }
    ...
    generateSectorBodies(sysName) {
        ...
        for(let i=0; i<count; i++) {
            ...
            let eco = "Unknown";
            if(pType.includes("Gas")) eco = "Refinery"; else if(pType.includes("Terran")) eco = "Agrarian";
            else if(pType.includes("Volcanic")) eco = "Industrial"; else eco = "Research";
            bodies.push({ name: `${sysName} ${['I','II','III','IV','V','VI'][i]}`, type: pType, economy: eco, desc: `A ${pType} world.`, mineable: mineable });
        }
        return bodies;
    }
    ...
    travel(target) {
        if(this.state.ship.stats.fuel_ion >= 10) {
            ...
            if (Math.random() < 0.25) {
                ...
                this.log("ALERT: Anomaly Detected.", "system");
            } else {
            ...
            this.log("Insufficient Ionic Fuel.", "system");
        }
    }
